...
const PAGE = `http://localhost:${PORT}/tests/smoke/stroke-buffer.html`;
...
async function main() {
  ...
  try {
    ...
    let u; if (!(await waitFor(async () => { const r = await fetch(`http://localhost:${DBG}/json/version`).then((x) => x.json()).catch(() => null); u = r?.webSocketDebuggerUrl; return !!u; }))) throw new Error("devtools did not start");
    ws = await new Promise((res, rej) => { const w = new WebSocket(u); w.onopen = () => res(w); w.onerror = rej; });
    const send = cdp(ws);
    const { targetId } = await send("Target.createTarget", { url: "about:blank" });
    const { sessionId } = await send("Target.attachToTarget", { targetId, flatten: true });
    const S = (m, p) => send(m, p, sessionId);
    await S("Page.enable"); await S("Runtime.enable"); await S("Page.navigate", { url: PAGE });
    ...
    if (!(await waitFor(() => E("window.__done === true"), 30000))) throw new Error("harness did not finish");
    const r = await E("window.__result");
    console.log("Faint line (alpha 0.4, width 4), darkness profile along the line:");
    console.log(`  direct  : spread ${r.directSpread}  CV ${r.directCV}  (joints darker → dots)`);
    console.log(`  buffered: spread ${r.bufferedSpread}  CV ${r.bufferedCV}  (uniform)`);
    console.log(`  mean darkness — direct ${r.directMean}, buffered ${r.bufferedMean}`);
    // buffered must be markedly more uniform than direct
    const ok = r.bufferedCV < r.directCV * 0.5 && r.bufferedSpread < r.directSpread * 0.5;
    console.log(ok ? "\n✓ PASS — buffered stroke is uniform; direct stroke dots at the joints" : "\n✗ FAIL");
    await send("Target.closeTarget", { targetId });
    return ok ? 0 : 1;
  } finally { try { ws?.close(); } catch {} br.kill("SIGKILL"); dev.kill("SIGKILL"); }
}
main().then((c) => process.exit(c)).catch((e) => { console.error("stroke-buffer failed:", e.message); process.exit(1); });
