...
const PORT = 4420, DBG = 9354;
const HERE = dirname(fileURLToPath(import.meta.url));
const PAGE = `http://localhost:${PORT}/tests/smoke/preset-io.html`;
...
async function main() {
  ...
  try {
    ...
    console.log(ok ? "\n✓ PASS — round-trips; zod rejects bad/empty/no-version/unknown-file/no-name" : "\n✗ FAIL");
    await send("Target.closeTarget", { targetId });
    return ok ? 0 : 1;
  } finally { try { ws?.close(); } catch {} br.kill("SIGKILL"); dev.kill("SIGKILL"); }
}
main().then((c) => process.exit(c)).catch((e) => { console.error("preset-io failed:", e.message); process.exit(1); });
