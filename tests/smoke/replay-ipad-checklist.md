...
(async () => {
  ...
  if (!init) { console.log('no init event — recording empty? rows =', rows.length); return; }
  ...
  const dims = { width: init.width, height: init.height, layers: init.layers, dpr: window.devicePixelRatio };
  // offscreen replay (compute + real Safari canvas raster) — build the world OUTSIDE the timed span
  const w = off.createOffscreenReplayWorld({ ...dims, store: seed() });
  const t0 = performance.now();
  eng.replay(rows, w.world);
  const offMs = performance.now() - t0;
  // compute-only floor (bare quadtree, no canvas)
